...
def entry_point():
	...
	for func_entry in func_list:
		...
		if statistic_instructions["count_bytes"] <= 10:
			continue
		...
		id_function: int = db.insert_data_for_function(name_function, hex(func_entry), statistic_instructions["count_bytes"], statistic_instructions["sum_identify"], graph, id_lib)
...
# def get_import_functions(ea, name, ord):
# 	if name:
# 		print("%08x: %s" % (ea, name))
...
# def for_dll():
# 	import_module_qty = idaapi.get_import_module_qty()
# 	for i in range(import_module_qty):
# 		module_name = idaapi.get_import_module_name(i)
# 		print(f'Модуль импорта: {module_name}')
# 		idaapi.enum_import_names(i, get_import_functions)
